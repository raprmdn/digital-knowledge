            ...
            <div class="widget_nav_menu">
                <ul>
                    @foreach ($categories as $category)
                        <li class="cat-item cat-item-2"><a href="{{ route('show.category', $category->category_slug) }}">{{ $category->category_name }}</a> <span class="post-count">{{ $category->articles_count }}</span></li>
                    @endforeach
                    @auth
